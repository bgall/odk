...
# - question_path: path to the 'survey' ODK sheet, must end in .csv (see below)
# - choices_path:  path to the 'choices' ODK sheet, must end in .csv (see below)
...
odk_to_codebook <- function(question_path, choices_path, csv_out, docx_out) {

#################################################
# Declare file paths for ODK question sheet
# and choices sheet. These sheets must be saved
# as .csv files. Additionally, the choice sheet
# value labels must be located in a column named
# "label::English". Both sheets can have a variety
# of other columns because the below script uses
# only required variables for the production
# of the ODK files into an XForm through 
# XLSForm and the other variables are dropped. 
#################################################

# File path to the main 'survey' ODK sheet
question <- as.character(question_path)

# File path to the 'choices' ODK sheet
choices <- as.character(choices_path)

#################################################
# Prepare ODK questions sheet
#################################################

# Read in ODK question sheet
question <- read.csv(question, stringsAsFactors = FALSE)

# First variable name is ALWAYS read in with extra
# characters from odd encodings. Since this column
# should alwas be the variable "type." we reset the
# value to be 'type'
colnames(question)[1] <- "type"
...
# Read in ODK question sheet
choices <- read.csv(choices, stringsAsFactors = FALSE)

# First variable name is read in with extra
# characters, so correct it to the correct name
colnames(choices)[1] <- "choice_list"

# Keep only variables for producing codebook
choices <-choices %>% select(choice_list,
                             val = name,
                             val_label = label..English)
...
#odk_to_codebook(question_path = "try.csv",
#                choices_path =  "try2.csv",
